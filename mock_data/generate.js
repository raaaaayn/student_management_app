...
const generate_faculty = () => {
  const faculty_id = faker.helpers.unique(faker.random.alphaNumeric, [7], {
    maxRetries: 200,
  });
...
const generate_students = (sems, students) => {
  for (const department of deps) {
    const related_sems = sems.filter(
    ...
    for (const semester of related_sems) {
      for (let i = 1; i <= faker.datatype.number({ min: 500, max: 999 }); i++) {
        ...
        const usn = `1EE${semester.scheme}${department}${String(i).padStart(
          3,
          "0",
        )}`;
...
const generate_students_subject_info = async (
  students,
  subjects,
  subject_infos,
  semesters,
...
const mock_data = () => {
  ...
  for (const [idx, dep] of deps.entries()) {
    const hod_id = faker.helpers.unique(faker.random.alphaNumeric, [7], {
      maxRetries: 200,
    });
    const name = faker.name.fullName();
    const role = "HOD";
  ...
  for (const dep of deps) {
    for (let sem_number = 1; sem_number <= 8; sem_number++) {
      const sem_id = faker.helpers.unique(faker.random.numeric, [4], {
        maxRetries: 200,
      });
      const department = dep;
      const faculties_wo_others = faculties.filter(
        (fac) => fac.role !== "Others",
      );
